 ...
 * Unsets known variables that cause issues rather than restoring to parent
 * enviroment One issue with restoring to the parent enviroment is that it unset
 * variables set by terminal emulators like TERM which need to be preserved in
 * the child shell
 *
 * This library also fixes a common issue when appimage portable home, config,
 * etc mode is used, where for example the HOME var from the portable .home dir
 * would be inherited by other processes launched by the appimage in portable
 * mode causing them to start using the fake .home dir instead of the real home
 *
 * It also offers the option to change argv0 of the running binary
 *
 * It also offers the ability to block specific libraries from being loaded via
 * dlopen by setting ANYLINUX_DO_NOT_LOAD_LIBS to a colon-separated list of glob
 * patterns
 *
...
__attribute__((constructor)) static void spoof_argv0(int argc, char **argv) {
...
// Fix host locale issues; mirrors the locale-check logic previously in
// AppRun-generic
...
// additional locales in in different locations, in those cases we do not
// intercept
...
__attribute__((constructor)) static void init_bindtextdomain_override(void) {
...
static int should_block_library(const char *filename) {
  ...
  while (token) {
    ...
    if (*token) {
      ...
      if (fnmatch(token, basename, 0) == 0 ||
          fnmatch(token, filename, 0) == 0) {
        blocked = 1;
        break;
      }
    }
...
static const char *vars_to_unset[] = {"BABL_PATH",
...
static int is_external_process(const char *filename) {
  ...
  int external =
      (strncmp(filename, appdir, MIN(strlen(filename), strlen(appdir))) != 0);
  DEBUG_PRINT("Process '%s' is %s (APPDIR=%s)\n", filename,
              external ? "EXTERNAL" : "INTERNAL", appdir);
  return external;
}

static int exec_common(execve_func_t function, const char *filename,
  ...
  if (is_external_process(path_to_check)) {
    ...
    const char *basename = strrchr(filename, '/');
    basename = basename ? basename + 1 : filename;
    if (strcmp(basename, "xdg-open") == 0 ||
        strcmp(basename, "gio-launch-desktop") == 0) {
      DEBUG_PRINT("Internal process detected (%s); cleaning environment anyway "
  ...
  if (ret == -1)
    DEBUG_PRINT("Underlying exec returned -1, errno=%d (%s)\n", errno,
                strerror(errno));
...
VISIBLE int execve(const char *filename, char *const argv[],
                   char *const envp[]) {
...
VISIBLE int execvpe(const char *filename, char *const argv[],
                    char *const envp[]) {
...
VISIBLE void *dlopen(const char *filename, int flags) {
  ...
  if (filename && should_block_library(filename)) {
    ...
    // If dlerror() returns NULL here the caller will segfault on the string
    // format. Trigger a real dlopen failure so the dynamic linker sets the
    // error state.
